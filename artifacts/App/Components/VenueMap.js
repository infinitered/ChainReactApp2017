    ...
    *************************************************************/
    constructor(props) {
        ...
        this.renderMapCloseButton = () => {
            ...
            return (React.createElement(TouchableOpacity, { onPress: this.props.onCloseMap, style: [styles.mapCloseButton, { left }] },
                React.createElement(Icon, { ref: 'mapCloseButton', name: 'times-circle', size: 26, color: Colors.purple, style: [styles.mapCloseButton] })));
        };
        /* ***********************************************************
        * STEP 1
    ...
    renderMapMarkers(location) {
        ...
        *************************************************************/
        return (React.createElement(MapView.Marker, { key: location.title, image: Images.markerIcon, coordinate: { latitude: location.latitude, longitude: location.longitude } },
            React.createElement(VenueMapCallout, { location: location, onPress: this.calloutPress })));
    }
    render() {
        return (React.createElement(View, null,
            React.createElement(MapView, { scrollEnabled: this.props.scrollEnabled, style: this.props.style, initialRegion: this.state.region, onRegionChangeComplete: this.onRegionChange, showsUserLocation: this.state.showUserLocation }, this.state.locations.map((location) => this.renderMapMarkers(location))),
            this.renderMapCloseButton()));
    }
}
