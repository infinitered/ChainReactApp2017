...
class TalkDetail extends React.Component {
    constructor() {
        ...
        this.renderSpeaker = (speaker, index) => {
            return (React.createElement(View, { key: index },
                React.createElement(Text, { style: styles.heading }, speaker.name),
                React.createElement(Text, { style: styles.description }, speaker.bio),
                React.createElement(View, { style: styles.social },
                    React.createElement(SocialMediaButton, { network: 'twitter', spacing: 'right', onPress: () => this.props.onPressTwitter(speaker.twitter) }),
                    React.createElement(SocialMediaButton, { network: 'github', spacing: 'right', onPress: () => this.props.onPressGithub(speaker.github) }))));
        };
    ...
    render() {
        return (React.createElement(PurpleGradient, { style: styles.linearGradient },
            React.createElement(ScrollView, null,
                React.createElement(View, { style: styles.container },
                    React.createElement(TouchableOpacity, { style: styles.backButton, onPress: this.goBack },
                        React.createElement(Image, { style: styles.backButtonIcon, source: Images.arrowIcon }),
                        React.createElement(Text, { style: styles.backButtonText }, "Back")),
                    React.createElement(View, { style: styles.cardShadow1 }),
                    React.createElement(View, { style: styles.cardShadow2 }),
                    React.createElement(Image, { style: styles.avatar, source: { uri: `https://infinite.red/images/chainreact/${this.props.image}.png` } }),
                    React.createElement(View, { style: styles.card },
                        React.createElement(Text, { style: styles.sectionHeading }, "TALK"),
                        React.createElement(Text, { style: styles.heading }, this.props.title),
                        React.createElement(Text, { style: styles.description }, this.props.description),
                        React.createElement(Text, { style: styles.sectionHeading }, "ABOUT"),
                        this.renderSpeakers()),
                    React.createElement(TalkInfo, { start: new Date(this.props.eventStart), duration: Number(this.props.duration), remindMe: this.isSpecial(), toggleRemindMe: this.toggleReminder, onPressGithub: this.props.onPressGithub, onPressTwitter: this.props.onPressTwitter, isFinished: this.props.currentTime > this.props.eventStart, showWhenFinished: false })))));
    }
}
