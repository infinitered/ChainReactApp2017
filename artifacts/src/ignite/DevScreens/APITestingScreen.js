...
export default class APITestingScreen extends React.Component {
    ...
    render() {
        return (React.createElement(View, { style: styles.mainContainer },
            ...
            React.createElement(ScrollView, { style: styles.container, ref: 'container' },
                React.createElement(View, { style: { alignItems: 'center', paddingTop: 60 } },
                    React.createElement(Image, { source: Images.api, style: styles.logo }),
                    React.createElement(Text, { style: styles.titleText }, "API")),
                React.createElement(View, { style: styles.section },
                    React.createElement(Text, { style: styles.sectionText }, "Testing API with Postman or APIary.io verifies the server works. The API Test screen is the next step; a simple in-app way to verify and debug your in-app API functions."),
                    React.createElement(Text, { style: styles.sectionText }, "Create new endpoints in Services/Api.js then add example uses to endpoints array in Containers/APITestingScreen.js")),
                this.renderButtons(),
                React.createElement(APIResult, { ref: 'result' }))));
    }
}
class APIResult extends React.Component {
    ...
    renderView() {
        return (React.createElement(ScrollView, { style: { top: 0, bottom: 0, left: 0, right: 0, position: 'absolute' }, overflow: 'hidden' },
            React.createElement(TouchableOpacity, { style: { backgroundColor: 'white', padding: 20 }, onPress: this.onApiPress },
                React.createElement(Text, null,
                    this.state.title,
                    " Response:"),
                React.createElement(Text, { allowFontScaling: false, style: { fontFamily: 'CourierNewPS-BoldMT', fontSize: 10 } }, this.state.message))));
    }
    render() {
